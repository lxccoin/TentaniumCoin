...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("35.189.217.236",80); // checkip.dyndns.org
...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("TentaniumCoin-ext-ip");
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TentaniumCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TentaniumCoin is probably already running."), addrBind.ToString().c_str());
        else
